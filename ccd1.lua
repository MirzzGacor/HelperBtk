...
if type(SendPacket) ~= "function" then SendPacket = function(...) end end
...
if type(SendVariant) == "function" then _SendVariant = SendVariant
elseif type(SendVariantList) == "function" then _SendVariant = SendVariantList
else _SendVariant = function(...) end end

local function SendVariantSafe(tbl, netid, delay)
    netid = netid or -1; delay = delay or 0
    local ok = pcall(function() _SendVariant(tbl, netid, delay) end)
    if not ok then pcall(function() _SendVariant(tbl) end) end
end
...
local function pkt_to_str(p)
    ...
    if type(p) == "table" then
        ...
        for k, v in pairs(p) do
            if type(v) == "string" then s = s .. v .. "|" else s = s .. tostring(v) .. "|" end
        end
        return s
    end
...
Tax = 5
leftd = false; rightd = false
local PX1, PY1, PX2, PY2
placeset = false; gem1 = false; gem2 = false
...
cvdl = false; pull = false; kick = false
bx = 0; by = 0; ban = false; autosb = false; punchset = false
...
function checkitm(id)
    for _, inv in pairs(GetInventory()) do if inv.id == id then return inv.amount end end
    return 0
end
...
function(packet)
    ...
    if pkt:find("/test (.+)") then
        ...
        SendVariantSafe(v); return true
    end

    if pkt:find("buttonClicked|blockSDB") then
        if blocksdb == false then blocksdb = true; ovlay("`2Block Sdb Mode Enabled")
        else blocksdb = false; ovlay("`4Block Sdb Mode Disabled") end
        return true
    end

    if pkt:find("action|dialog_return\ndialog_name|kk\nbuttonClicked|Wrench") then
        local dlg = {}; dlg[0] = "OnDialogRequest"; dlg[1] = wrenchop; SendVariantSafe(dlg); LogToConsole("Wrench Option"); return true
    end

    if pkt:find("buttonClicked|pos") then leftd = true; punchset = true; ovlay("`4Punch Left Display") end
    if pkt:find("buttonClicked|gems") then gem1 = true; placeset = true; ovlay("`oPlace Left Chandelier\nif Horizontal\n[Chand][`4Put Chand Here`o][Chand]\nif Vertical VVVV\n[Chand]\n[Chand]\n[`4Put Chand Here`o]"); tol("\n`oPlace Left Chandelier...") end

    if pkt:find("action|dialog_return\ndialog_name|kk\nbuttonClicked|Proxy") then
        local dlg = {}; dlg[0] = "OnDialogRequest"; dlg[1] = proxy; SendVariantSafe(dlg); LogToConsole("Fiture I Will Add"); return true
    end
    ...
    if pkt:find("/d (%d+)") then local txt = pkt:match("/d (%d+)"); DropItem(1796, txt); tol("Succes Drop `0"..tostring(txt).." `2Diamond Lock"); return true end
    if pkt:find("/w (%d+)") then local txt = pkt:match("/w (%d+)"); DropItem(242, txt); tol("Succes Drop `0"..tostring(txt).." `2World Lock"); return true end

    if pkt:find("/hitam") then SendPacket(2, "action|setSkin\ncolor|0000000000"); SendPacket(2, "action|input\n|text|`4ORA PERLU GANTENG SING PENTING IRENG!"); return true end
    if pkt:find("/putih") then SendPacket(2, "action|setSkin\ncolor|510000000000"); SendPacket(2, "action|input\n|text|`0ORA PERLU GANTENG SING PENTING PUTIH!"); return true end

    if pkt:find("/bdl") then
        if cvdl == false then cvdl = true; ovlay("`2Buy Dl Mode Enable")
        else if cvdl == true then cvdl = false; ovlay("`4Buy Dl Mode Disable"); return true end end
    end

    if pkt:find("/b (%d+)") then local txt = pkt:match("/b (%d+)"); DropItem(7188, txt); tol("`2Succes Drop `0"..tostring(txt).." `2Blue Gem Lock"); return true end

    if pkt:find("/ww (.+)") or pkt:find("/Ww (.+)") then
        local namew = pkt:match("/ww (.+)") or pkt:match("/Ww (.+)")
        ovlay("`#Warping To `6"..tostring(namew)); SendPacket(3, "action|join_request\n|name|"..tostring(namew).."\n|invitedWorld|0"); return true
    end

    if pkt:find("/bb (%d+)") or pkt:find("/Bb (%d+)") then
        if checkitm(11550) == 0 then SendPacket(2,"action|dialog_return\ndialog_name|info_box\nbuttonClicked|make_bgl\n"); return true end
    end

    if pkt:find("/bb (%d+)") or pkt:find("/Bb (%d+)") then
        local txt = pkt:match("/bb (%d+)") or pkt:match("/Bb (%d+)"); DropItem(11550, txt); tol("`2Succes Drop `0"..tostring(txt).." `bBlack Gem Lock"); return true
    end
    ...
    if pkt:find("\nbuttonClicked|ra") then
        if by == 0 then ovlay("U Didnt Set Back Pos Lol Do /setb")
        else resetchand1 = true; ovlay("Put Chand Wait"); return true end
    end

    if pkt:find("\nbuttonClicked|rd") then
        if by == 0 then ovlay("U Didnt Set Back Pos Lol Do /setb")
        else resetchand2 = true; ovlay("Put Chand Wait"); return true end
    end

    if pkt:find("friends") then local dlg = {}; dlg[0] = "OnDialogRequest"; dlg[1] = lol; SendVariantSafe(dlg); tol("Proxy List"); return true end
    if pkt:find("store") then local dlg = {}; dlg[0] = "OnDialogRequest"; dlg[1] = tap; SendVariantSafe(dlg); tol("Btk Helperr"); return true end

    if pkt:find("/pos") or pkt:find("/Pos") then pos(); return true end

    if pkt:find("/cd (%d+)") or pkt:find("/Cd (%d+)") then
        Amount = pkt:match("/cd (%d+)") or pkt:match("/Cd (%d+)")
        LogToConsole("`9Use Fitur : /cd")
        bgl = math.floor(Amount/10000); Amount = Amount - bgl*10000
        dl = math.floor(Amount/100); wl = Amount % 100
        AutoBtk = true
        local hasil = (bgl ~= 0 and bgl.." `eBlue Gem Lock`0" or "").." "..(dl ~= 0 and dl.." `1Diamond Lock`0" or "").." "..(wl ~= 0 and wl.." `9World Lock`0" or "")
        tol("`9Total drop : `0"..tostring(hasil)); return true
    end

    if pkt:find("/stax (%d+)") or pkt:find("/Stax (%d+)") then Tax = pkt:match("/stax (%d+)") or pkt:match("/Stax (%d+)"); ovlay("Tax : "..tostring(Tax).."%") end

    if pkt:find("\nbuttonClicked|tk") then
        if Tax == 0 then ovlay("`4Set Tax First")
        else colect(); tax = math.floor(Amount * Tax / 100); drop = Amount - tax; bets = Amount//2
            tol("`2Tax : `"..tostring(Tax).."%, `4Total drop : `9"..tostring(drop)); tol("`9Succes Take")
    ...
    if pkt:find("\nbuttonClicked|w1") then
        if Tax == 0 then ovlay("`4Set Tax First")
        else bgl = math.floor(drop/10000); drop = drop - bgl*10000; dl = math.floor(drop/100); wl = drop % 100
            SendPacketRaw(false, { type = 0, x = (PX1 - 2) * 32, y = (PY1) * 32, state = 32 })
            AutoBtk = true
            local hasil = (bgl ~= 0 and bgl.." `eBlue Gem Lock`0" or "").." "..(dl ~= 0 and dl.." `1Diamond Lock`0" or "").." "..(wl ~= 0 and wl.." `9World Lock`0" or "")
            tol("`9Amount Lock : "..tostring(Amount)); tol("`9Tax : "..tostring(Tax).."%"); tol("`9Total drop : `0"..tostring(hasil).." `4Tax Reset"); return true
        end
    end
    ...
    if pkt:find("/rr") then relog = true; namew = GetWorld().name; return true end
...
function(v)
    ...
    if v0 == "OnConsoleMessage" and v1:find("commands.") then
        tol("`cIDK What Command U Use There Was No Have Command Like That "); tol("`9Type /fitur to show feature"); return true
    end
    ...
    if v0 == "OnTalkBubble" and v2:find("spun the wheel and got") and (v2:find("`4(%d+)``!") or v2:find("`b(%d+)``!") or v2:find("`2(%d+)``!")) then
        ...
        if SpunNumber then
            ...
            v[0] = "OnTalkBubble"; v[1] = v[1]; v[2] = v[2] .. " (cool)Fast `0 `2Reme`0 [ " .. Reme .. "`0 ]"
            SendVariantSafe(v); return true
        end
    end

    if v0 == "OnSDBroadcast" then if blocksdb == true then ovlay("`#Va:VanzCya `9I Blocked Sdb Say Thanks To me"); return true end end

    if v0:find("OnDialogRequest") then if v1:find("Wow, that's fast delivery.") then ovlay("Block Dialog Telephone"); return true end end
            ...
            for _, display in pairs(GetTiles()) do
                if display.x == a.px and display.y == a.py then
                    if gem1 and not gem2 and not leftd and not rightd then
                        ...
                        tile = {
                            pos1 = { {x = xgem1, y = ygem1}, {x = xgem1, y = ygem2}, {x = xgem1, y = ygem3}, {x = xgem2, y = ygem1}, {x = xgem3, y = ygem1} },
                            pos2 = { {x = xgemm1, y = ygemm1}, {x = xgemm1, y = ygemm2}, {x = xgemm1, y = ygemm3}, {x = xgemm2, y = ygemm1}, {x = xgemm3, y = ygemm1} }
                        }
...
while true do
    ...
    if AutoBtk then
        Sleep(200)
        if checkitm(1796) < dl then wear(7188); Sleep(500) end
        if checkitm(242) < wl then wear(1796); Sleep(500) end
        if bgl and bgl > 0 then DropItem(7188, bgl); Sleep(500) end
        if dl and dl > 0 then DropItem(1796, dl); Sleep(500) end
        if wl and wl > 0 then DropItem(242, wl) end
        drop = ""; Amount = ""; AutoBtk = false
    end

    if resetchand1 then
        Sleep(500); FindPath(xgem1, ygem2); ChangeFeature("Modfly", true); Sleep(200)
        local pkt = { type = 3, x = GetLocal().pos.x, y = GetLocal().pos.y, px = xgem1, py = ygem1, value = 5640, state = 16 }
        SendPacketRaw(false, pkt); Sleep(200)
        ...
        SendPacketRaw(false, pkt); colecp1(); Sleep(200)
        FindPath(xgemm1, ygemm2); Sleep(200)
        ...
        SendPacketRaw(false, pkt); colecp2(); Sleep(200)
        FindPath(bx, by); ChangeFeature("Modfly", false); resetchand1 = false
    end

    if resetchand2 then
        Sleep(500); FindPath(xgem1 + 2, ygem1); Sleep(100); FindPath(xgem1, ygem1); Sleep(250)
        local pkt = { type = 3, x = GetLocal().pos.x, y = GetLocal().pos.y, px = xgem1, py = ygem1, value = 5640, state = 16 }
        SendPacketRaw(false, pkt); Sleep(250)
        ...
        SendPacketRaw(false, pkt); colecp3(); Sleep(500)
        FindPath(xgem1 + 4, ygem1); Sleep(100); FindPath(xgemm1 - 2, ygemm1); Sleep(100); FindPath(xgemm1, ygemm1); Sleep(250)
        pkt = { type = 3, x = GetLocal().pos.x, y = GetLocal().pos.y, px = xgemm1, py = ygemm1, value = 5640, state = 16 }
        SendPacketRaw(false, pkt); Sleep(250)
        ...
        SendPacketRaw(false, pkt); colecp4(); Sleep(500)
        FindPath(bx, by); resetchand2 = false
    end

    if relog then SendPacket(3,"action|quit_to_exit"); Sleep(100); SendPacket(3, "action|join_request\n|name|"..tostring(namew).."\n|invitedWorld|0") end
    relog = false
end
